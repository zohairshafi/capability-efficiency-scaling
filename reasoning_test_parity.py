...
from reasoning_task import Parity
...
size = int(sys.argv[1])     # number of bits in the parity sequence
...
def main():
    ...
    for i in range(num_instances):
        bits = problem_rng.integers(0, 2, size=size)
        problem_instance = Parity(bits=bits)
    ...
    engine = sgl.Engine(
        ...
        dtype="bfloat16",
        mem_fraction_static=0.8,  # reserve more memory for KV-cache (less fragmentation with large models)
    ...
    for temperature in [0.6]:
        ...
        for i, output in enumerate(outputs):
            ...
            results['results'].append({
                ...
                'output': output_text,
                'size': int(size),
                'correct_answer': correct_answers[i],  # str: 'even' or 'odd'
        ...
        with open(f"{result_dir}result_size={size}_parity-temp={temperature}-flash={using_flash}_sglang_batched_fp32.pkl", "wb") as f:
