...
from reasoning_task import IndexLookup
...
size = int(sys.argv[1])     # length of the sequence
min_value = 0               # smallest value in the sequence (constant bandwidth)
max_value = 100             # largest value in the sequence
...
def main():
    ...
    for i in range(num_instances):
        sequence = problem_rng.integers(low=min_value, high=max_value + 1, size=size)
        position = int(problem_rng.integers(0, size))
        problem_instance = IndexLookup(sequence=sequence, position=position)
    ...
    engine = sgl.Engine(
        ...
        dtype="bfloat16",
        mem_fraction_static=0.85,  # reserve more memory for KV-cache (less fragmentation with large models)
    ...
    for temperature in [0.6]:
        ...
        with open(f"{result_dir}result_size={size}_index-temp={temperature}-flash={using_flash}_sglang_batched_fp32.pkl", "wb") as f:
