...
from reasoning_task import ModularAddition
...
def main():
    ...
    for i in range(num_instances):
        numbers = problem_rng.integers(low=min_value, high=max_value + 1, size=size)
        problem_instance = ModularAddition(numbers, modulus=modulus)
        ...
        correct_answers.append(problem_instance())
    ...
    for p in raw_prompts:
        if 'deepseek-v4' in model_name.lower():
            # DeepSeek V4 uses special tokens; no Jinja chat template.
    ...
    for temperature in [0.6]:
        ...
        with open(f"{result_dir}result_size={size}_value={min_value}-{max_value}_mod={mod_str}-temp={temperature}-flash={using_flash}_sglang_batched_fp32.pkl", "wb") as f:
